...
/// disk.
pub fn render_to<W: Write, S: SimplexTrie + IntoSimplexDimIter<Item = Simplex>>(
    complex: &S,
    output: &mut W,
) {
...
impl<S: SimplexTrie + IntoSimplexDimIter<Item=Simplex>> From<&S> for Graph {
    fn from(complex: &S) -> Graph {
        ...
        for simplex in complex.iter_dim(1) {
            match simplex.dim() {
                0 => {
